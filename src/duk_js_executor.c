...
#define DUK__MASK_A     (0xffUL << DUK__TVAL_SHIFT)  /* FIXME: masks */
#define DUK__MASK_B     (0x1ffUL << DUK__TVAL_SHIFT)
#define DUK__MASK_C     (0x1ffUL << DUK__TVAL_SHIFT)
#define DUK__MASK_BC    (0x3ffffUL << DUK__TVAL_SHIFT)
...
DUK_LOCAL DUK_NOINLINE void duk__js_execute_bytecode_inner(duk_hthread *entry_thread, duk_size_t entry_callstack_top) {
	 ...
	 * either 8 or 16.  If that's not the case, disable FIXME.
	 */
#if 1
#if defined(DUK_USE_PACKED_TVAL)
 ...
 restart_execution:
	...
	for (;;) {
		...
		/* FIXME: flag check directly against 'ins' */
...
#undef DUK__RCISREG_B
#undef DUK__RCISREG_C
